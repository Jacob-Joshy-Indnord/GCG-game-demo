	...
	bool bounce, bool sphere, bool bounceLess)
{
	...
	if (ca.mType == CAM_Helicopter)
	{
		...
		Vector3 goalPos = posGoal - carFwd * trailD + Vector3(0.f, heliH, 0.f);
		...
		else
		{
			...
			mHeliPos += (goalPos - mHeliPos) * t;
		}
		camPosFinal = mHeliPos;

		//  orientation: look at the car from above
...
bool FollowCamera::loadCameras()
{
	...
	{	CameraView hc;
		...
		hc.mMain  = 0;       // visible in main cycling
